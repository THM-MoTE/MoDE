...
import de.thm.mni.mote.mode.ui.control.NotificationControl;
import de.thm.mni.mote.mode.util.logger.Message;
...
public abstract class NotifyController extends Controller implements Observer {
  ...
  private void addNotification(final NotificationControl notification) {
    ...
    fadeIn.setOnFinished(e1 -> {
      PauseTransition delay = new PauseTransition(Duration.seconds(settings.getNotification().getSeconds()));
