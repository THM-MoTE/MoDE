 ...
 * @declaredat /home/hobbypunk/Dokumente/Entwicklung/THM_Projekte/Projektphase/jModelica/Compiler/ModelicaMiddleEnd/src/jastadd/structural/DynamicStates.jrag:263
 */
public class IndexReductionBiPGraph extends AbstractBiPGraph<IndexReductionEq, IndexReductionVar, IndexReductionSCCBlock> {
    ...
    public Set<IndexReductionVar> computeDynamicStateSets(Map<IndexReductionEq, Map<IndexReductionVar, Coefficient>> coefficientsMatrix, IndexReductionResult result) {
        ...
        if (skipDynamicStates) {
            ...
            for (Entry<IndexReductionVar, IndexReductionEq> entry : initialMatching.entrySet()) {
    ...
    public FVariable.StateSelect computeIncidenceStateSelect(IndexReductionEq eq, IndexReductionVar var) {
        ...
        for (FIdUseExp exp : eq.getEquation().findFIdUseExpsInTree()) {
    ...
    public Object printCoefficientsObj(final Map<IndexReductionEq, Map<IndexReductionVar, Coefficient>> coefficientsMatrix, final boolean printExpressions) {
        ...
        @Override
        protected IndexReductionEq createEq(String name, FAbstractEquation eqn, int groupNumber, Collection<IndexReductionEq> groupMembers, FTypePrefixVariability variability, FType type, boolean isMeta) {
        ...
        private class ExtraEquationVariableResolver implements VariableResolver<IndexReductionEq, IndexReductionVar> {
            ...
            private ExtraEquationVariableResolver(Collection<FAbstractEquation> equations) {
                ...
                for (FAbstractEquation equation : equations) {
                    ...
                    for (FIdUseExp lhs : incidenceMap.assigned()) {
            ...
            @Override
            public Set<IndexReductionVar> lookupVars(Iterable<FIdUseExp> set) {
                Set<IndexReductionVar> res = new LinkedHashSet<IndexReductionVar>();
                lookupVars(set, res);
                return res;
            }
    
            private void lookupVars(Iterable<FIdUseExp> set, Set<IndexReductionVar> resolvedVars) {
    ...
    private void differentiateEquation(IndexReductionEq baseEqn,
        ...
        ASTNode.log.verbose("Differentiating %s", baseEqn);
        ...
        for (FAbstractEquation newEqn : originalDiffedFEqns) {
            ...
            for (int i = 0; i < newEqn.numScalarEquations(); i++) {
                ...
                ASTNode.log.verbose("About to add equation: %s", newEqn);
    ...
    public IndexReductionResult reduceIndex(IndexReductionFunctionInliner inliner,
            boolean computeDynamicStates, int maximumNbrOfMunkresIncidences, boolean printPantelidesSteps) {
        ...
        while (!eqToMatchStack.empty() ){
            ...
            ASTNode.log.verbose("*************");
            ASTNode.log.verbose("Looking for augmented path starting in equation: %s", eqToMatch);
            if (pathFound)
                ASTNode.log.verbose("Path found!");
            else
                ASTNode.log.verbose("Path not found!");
            
            ASTNode.log.verbose("Visisted equations:");
            for (IndexReductionEq ee : visitedEquations)
                ASTNode.log.verbose("  %s", ee.printEquation());
            
            ASTNode.log.verbose("Visisted variables:");
            for (IndexReductionVar vv : visitedVariables)
                ASTNode.log.verbose("  %s", vv);
            ...
            if (!pathFound) { // Step 3b-5
                ...
                if (printPantelidesSteps) {
                    ASTNode.log.verbose("Graph after addition of equations and variables:");
                    ASTNode.log.verbose(this);
                }
            }
            if (printPantelidesSteps) {
                ASTNode.log.verbose("Matching:");
                ASTNode.log.verbose(printMatchingObj());
                ASTNode.log.verbose("*************");
            }
        }
        ...
        ASTNode.log.verbose("Index reduction done!");
        ASTNode.log.verbose(this);
        ASTNode.log.verbose("Matching:");
        ASTNode.log.verbose(printMatchingObj());
        ...
        if (result.getEquations().isEmpty()) {
            ASTNode.log.verbose("Index reduction did not change the system. Munkres is skipped.");
            return result;
        }
        ...
        */
        ...
        Collection<IndexReductionSCCBlock> components = tarjan(fclass.myOptions(), true);
        ...
        for (IndexReductionSCCBlock block : components) {
            if (block.containsDifferentatedComponents()) {
                ASTNode.log.verbose(" ********* Found block containing diffed equations ***********");
                ASTNode.log.verbose(block);
        ...
        while (remainingDifferentiatedEquations) {
            ...
            for (IndexReductionVar var : getVariables()) {
                ...
                ASTNode.log.verbose("Weight: %s %s", var, weight);
            ...
            if (computeDynamicStates)
                dsVars = computeDynamicStateSets(coefficientsMatrix, result);
            ASTNode.log.verbose(printMatchingObj());
            ...
            // to zHat.
            Collection<IndexReductionVar> varsToRemove = new ArrayList<IndexReductionVar>();
            ASTNode.log.verbose(" Dummy derivatives selected in iteration: ");
            for (IndexReductionVar var : new ArrayList<IndexReductionVar>(getVariables())) {
                varsToRemove.add(var);
        ...
        ASTNode.log.verbose(" Dummy derivatives: ");
        ...
        public enum COEFFICIENT_STATE{
            ...
            public static COEFFICIENT_STATE create(FTypePrefixVariability variability) {
                if (variability == null)
                    return EXCEPTION;
    ...
    public void matchMunkres(Map<IndexReductionVar, IndexReductionCost> weights) {
        ...
        ASTNode.log.verbose("Munkres result:");
        for (int i = 0; i < result.length; i++) {
            ...
            ASTNode.log.verbose("%s: %s %s", eqn, var, cost[result[i][0]][result[i][1]]);
        }
        ASTNode.log.verbose("Optimal cost: %s", opt_cost);
    }
